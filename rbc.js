...
function renderRbcCarga() {
  ...
  tbody.innerHTML = R.pontos.map((p, i) => {
    ...
    return `<tr data-carga="${p.carga}">
      ...
      <td>${rotPesos}<br><span class="rbc-link" onclick="abrirModalPesos(${i})">＋ escolher pesos</span></td>
...
// adicionar/remover
function addCargaRbc() { window._rbc.pontos.push({ carga:'', leituras:Array(window._rbc.numLeituras).fill(''), pesos:[], orcamento:null }); renderRbcCarga(); }
