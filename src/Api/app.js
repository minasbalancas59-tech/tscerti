...
async function irPainel() {
  ...
  if (gestor) { renderGraficos(graficosDias); avisoContrato(); cardPlano(); }
  guiaPrimeirosPassos();
  const certs = await api('/certificados');
  certsPainelCache = certs;
...
function htmlListaGestor(certs) {
  ...
  const filtros = `<div class="barra-btns" style="margin-bottom:10px;flex-wrap:wrap">
      ...
      <span style="border-left:1px solid #d5dde5;margin:0 4px"></span>
      ...
      <input type="search" id="busca-os" value="${esc(filtroOS)}" placeholder="📋 ordem de serviço"
        ...
        style="padding:5px 9px;border:1px solid #dde5ec;border-radius:7px;font:inherit;
               font-size:.85rem;max-width:150px">
    </div>`;
    ...
    return `
      <div class="item-cert${c.emitir_rbc ? ' item-rbc' : ''}" onclick="abrirCert('${c.id}','${c.status}')">
        <span><b>${esc(c.cliente)}</b> · ${esc(c.balanca)}
          ...
          <br><span class="dica">Téc.: ${esc(c.tecnico)}${c.numero ? ' · ' + c.numero : ''}</span>
...
async function etiquetaDoRascunho() {
  if (!certId) { toast('Salve o rascunho antes de imprimir a etiqueta.', 'erro'); return; }
  await salvarRascunho(false);
  imprimirEtiqueta(certId);
}
...
async function imprimirNiimbot(tam, sempreMostrar = false) {
  ...
  if (!navigator.bluetooth) {
    toast('Este navegador não conversa com a impressora por Bluetooth. ' +
          'Use o Chrome ou o Edge no computador ou no Android — no iPhone não funciona.',
          'erro', 9000);
    return;
  }
...
async function tentarImpressaoDireta(tam, cfg) {
  ...
  try {
    ...
    const nb = _niimbot;
    const nome = nb.caract ? (nb.dispositivo?.name || 'impressora')
                           : await nb.reconectar();
...
async function enviarFotoCert(input) {
  ...
  for (const arquivo of arquivos) {
    ...
    try {
      ...
      $('#foto-erro').textContent = `${e.message}${arquivos.length > 1 ? ` (${enviadas} de ${arquivos.length} enviadas)` : ''}`;
      break;
    }
  }
  if (enviadas) {
...
async function enviarFotoEnsaio(input) {
  ...
  // garante que o rascunho está salvo (o certificado precisa existir no banco)
  try { await salvarRascunho(false); } catch { /* segue */ }
  const legenda = prompt('Legenda da foto (opcional) — ex.: "Carga 10 kg":') || '';
  const fd = new FormData();
  fd.append('arquivo', arquivo);
  if (legenda) fd.append('legenda', legenda);
  try {
    ...
    carregarFotosEnsaio();
  } catch (e) { $('#ens-foto-erro').textContent = e.message; }
  input.value = '';
}
...
async function abrirRevisao(id) {
  ...
  const html = `
    <div class="card">
      ...
      <table><thead><tr><th class="num">Carga</th>${temAjuste ? '<th class="num">Antes ajuste</th>' : ''}<th class="num">${temAjuste ? 'Após ajuste' : 'Indicação'}</th><th class="num">Erro</th>
      ...
      ${d.excentricidade && d.excentricidade.length > 0 ? `
        ...
        <table><thead><tr><th>Posição</th><th class="num">Carga</th>${excAntes ? '<th class="num">Antes ajuste</th>' : ''}<th class="num">${excAntes ? 'Após ajuste' : 'Indicação'}</th><th class="num">Erro (vs centro)</th><th>Situação</th></tr></thead>
  ...
  $('#sa-conteudo').innerHTML = `
    ...
    <div class="saude-cards">
      <div class="saude-card" style="border-left:4px solid #17724a">
        ...
        <span class="dica">ativos nos últimos 5 min</span></div>
...
async function renderEmailLogSA(filtros = {}) {
  ...
  const linhas = lista.map(l => `
    <tr class="${l.status === 'erro' ? 'linha-erro-email' : ''}">
      ...
      <td><b>${esc(l.destinatario)}</b>${l.nome_destino ? `<br><span class="dica">${esc(l.nome_destino)}</span>` : ''}</td>
      <td>${l.empresa ? esc(l.empresa) : '<span class="dica">sistema</span>'}
          ${l.cliente ? `<br><span class="dica">${esc(l.cliente)}</span>` : ''}</td>
      <td class="dica">${esc(l.assunto)}${l.certificado_numero ? `<br>${esc(l.certificado_numero)}` : ''}
          ${l.erro_detalhe ? `<br><span class="erro" style="font-size:.8rem">${esc(String(l.erro_detalhe).slice(0,120))}</span>` : ''}</td>
  ...
  $('#sa-conteudo').innerHTML = `
    ...
    <div class="card" style="padding:16px 18px">
      ...
      <div style="overflow-x:auto;margin-top:10px">
        <table class="tab-sa">
          <thead><tr><th>Quando</th><th>Status</th><th>Motivo</th><th>Destinatário</th><th>Empresa/Cliente</th><th>Assunto / erro</th></tr></thead>
          <tbody>${linhas}</tbody>
        </table>
      </div>
...
function wizEmpresaRender() {
  ...
  if (w.passo === 1) html = `
    ...
    <div class="form-grid">
      ...
      <label>Cidade/UF ${w.avaliacao ? '' : '*'} <input type="text" id="we-cidadeuf" value="${e(w.cidadeuf)}" placeholder="Contagem/MG"></label>
...
async function visualizarEmpresa(id, nome) {
  const ok = await modalConfirmar(
    '👁 Visualizar dados da empresa',
    `Você vai entrar no modo de visualização (somente leitura) da empresa "${nome}".\n\n` +
    'Você verá os dados como a empresa vê, mas não poderá alterar nada. ' +
    'Este acesso fica registrado. Deseja continuar?',
    { textoSim: 'Entrar na visualização', textoNao: 'Cancelar' });
  if (!ok) return;
  try {
    const r = await saApi('/empresas/' + id + '/visualizar', { method: 'POST' });
    // Troca para o token de visualização
    token = r.token;
    localStorage.setItem('token', token);
    // Atualiza o "usuario" local para refletir o modo visualização
    usuario = { ...usuario, papel: 'responsavel_tecnico', empresa: r.empresaNome,
      _visualizando: true, _empresaVis: r.empresaNome };
    localStorage.setItem('usuario', JSON.stringify(usuario));
    localStorage.setItem('_visualizando', '1');
    mostrarBannerVisualizacao(r.empresaNome);
    irPainel();
  } catch (e) { toast(e.message, 'erro'); }
}

function mostrarBannerVisualizacao(nomeEmpresa) {
  document.getElementById('banner-visualizacao')?.remove();
  const banner = document.createElement('div');
  banner.id = 'banner-visualizacao';
  banner.innerHTML = `
    <span>👁 <b>Modo visualização</b> — você está vendo os dados da empresa
      <b>${esc(nomeEmpresa)}</b> (somente leitura).</span>
...
async function abrirEmpresaSA(id) {
  ...
  $('#sa-conteudo').innerHTML = `
    <div class="barra">
      ...
      <div class="barra-btns">
        <button class="btn-primario" onclick="visualizarEmpresa('${id}', '${esc(e.razao_social).replace(/'/g, "\\'")}')">👁 Visualizar dados</button>
  ...
  return `<td><input type="number" step="any" inputmode="decimal" class="in-carga"
    ...
    <td><div class="ind-wrap">
      <input type="number" step="any" inputmode="decimal" class="in-indic"
             value="${fmtCampo(indic)}" oninput="recalcular()" onblur="arredondarCampo(this)">
...
async function enviarAprovacao() {
  ...
  document.querySelectorAll('#tab-indicacao tbody tr').forEach(tr => {
    ...
    const inp = tr.querySelector('.in-indic');
    if (temCarga && num(inp?.value) == null) {
    ...
    if (ajusteAtivo && temCarga) {
      const antes = tr.querySelector('.in-antes');
      const colAntes = tr.querySelector('.col-antes');
      if (antes && colAntes && colAntes.style.display !== 'none' && num(antes.value) == null) {
  ...
  // Precisa de ao menos 3 indicações preenchidas
  const preenchidas = [...document.querySelectorAll('#tab-indicacao .in-indic')]
    .filter(i => num(i.value) != null).length;
  if (preenchidas < 3) {
...
if (location.hash.startsWith('#convite=')) mostrar('tela-convite');
  ...
  if (claimDoToken('impersonando') === 'true') {
    ...
    localStorage.setItem('_visualizando', '1');
    mostrarBannerVisualizacao(nome);
  } else {
