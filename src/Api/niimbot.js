...
class NiimbotWeb {
  ...
  async reconectar() {
    if (NiimbotWeb.ultimoDispositivo) {
      try {
        const n = await this.abrirGatt(NiimbotWeb.ultimoDispositivo);
        if (n) return n;
      } catch (e) { /* segue para o getDevices */ }
    }
    if (!navigator.bluetooth?.getDevices) return null;
    let conhecidos = [];
    try { conhecidos = await navigator.bluetooth.getDevices(); } catch (e) { return null; }
    for (const dev of conhecidos) {
      try { const n = await this.abrirGatt(dev); if (n) return n; }
      catch (e) { this.caract = null; }
    }
    return null;
  }
  ...
  async enviar(cmd, dados) {
    ...
    await this.caract.writeValueWithoutResponse(this.montar(cmd, dados));
  }
