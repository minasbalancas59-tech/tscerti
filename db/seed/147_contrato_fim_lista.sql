-- Lista de empresas do SA: + contrato_fim (data de fim do contrato ativo),
-- para o filtro/badge "Contrato vencendo". Joao, 22/08/2026.
...
CREATE FUNCTION public.sa_listar_empresas()
 RETURNS TABLE(id uuid, razao_social text, cnpj text, plano text, status text,
               ...
               nome_fantasia text, tem_contrato boolean, contrato_fim date)
 LANGUAGE sql
 SECURITY DEFINER
...
AS $function$
    SELECT e.id, e.razao_social, e.cnpj,
           ...
           (SELECT ct3.fim FROM contrato ct3
             WHERE ct3.empresa_id = e.id AND ct3.ativo
             ORDER BY ct3.inicio DESC LIMIT 1)
      FROM empresa e
